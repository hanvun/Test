...
public class PlayerController : MonoBehaviour {
	...
	//プレイヤーから地面までの下の距離
	private float groundUnderDistance = 0.15f;
	//プレイヤーが落下までの距離
	private float FalingDistance = 0.1f;
	//プレイヤーから地面までの左までの距離
	private float groundLeftDistance = 0.15f;
	//プレイヤーから地面までの右までの距離
	private float groundRightDistance = 0.15f;
	...
	void jump( ){
		...
		if ( isJump ) {
			...
			if (!GroundJudge (Vector2.up, groundUnderDistance)) {
				//空中に現在の速度分移動
